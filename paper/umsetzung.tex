...
Die Rückprojektion erfolgt für jede Projektion $p$ einzeln und für ein auf dem \gls{device} erstelltes dreidimensionales
Volumen (vgl. Abschnitt~\ref{sssec:cu_mem}). Der Implementierung (siehe Quelltext~\ref{source:impl_bp}) liegt dabei die
in Abbildung~\ref{fig:fdk_geometrie} gezeigte Geometrie zugrunde. Für jedes \gls{voxel} im Volumen müssen zunächst die
zugehörigen Projektionskoordinaten unter dem Winkel $\alpha_p$ bestimmt werden. 
